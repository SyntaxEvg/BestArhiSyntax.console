...
namespace Sender.Services.Services
{
    ...
    public class EmailWriter : IEmailWriter
    {
        ...
        {
            if (addresses is null || (addresses is not null && !addresses.Contains("@"))) return this;
            var mailboxAddresses = addresses.ParseEmailContacts();
            _mimeMessage.From.AddRange(mailboxAddresses);
            return this;
        }

        public IEmailWriter ReplyTo(string name, string addresses)
        {
            if (addresses is null || (addresses is not null && !addresses.Contains("@"))) return this;
            _mimeMessage.ReplyTo.Add(new MailboxAddress(name, addresses));
            return this;
        }
        ...
        public IEmailWriter To(string name, string addresses)
        {
            if (addresses is null || (addresses is not null && !addresses.Contains("@"))) return this;
            _mimeMessage.To.Add(new MailboxAddress(name, addresses));
            return this;
        }
        ...
        public IEmailWriter BodyHtml(string textHtml)
        {
            if (textHtml is not null  && textHtml.Contains(">"))
            {
                return Body(textHtml, true);
            }
            return Body(textHtml, false);
        }
        ...
        public IEmailWriter Attach(Stream stream, string fileName, string contentType = null, string contentId = null, bool linkedResource = false)
        {
            ...
            return Attach(GetMimePart(stream, fileName, contentType, contentId), linkedResource);
        }
        ...
        public (bool,string) TrySend(CancellationToken cancellationToken = default) =>
            TrySendAsync(cancellationToken).ConfigureAwait(false).GetAwaiter().GetResult();

        public async Task<(bool,string)> TrySendAsync(CancellationToken cancellationToken = default, ITransferProgress transferProgress = null)
        {
            var isSent = await _emailClient.TrySendAsync(_mimeMessage, cancellationToken, transferProgress).ConfigureAwait(false);
            if (!isSent.Item1)
            {
                return isSent;
            }
